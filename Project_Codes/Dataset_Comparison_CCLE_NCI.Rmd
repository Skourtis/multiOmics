...
map(unique(IntraFeature_CCLE$Tissue_Intracorrelation_df$Pathway),
  ...
  mutate(Colour = if_else(.$Pathway == .x,.x,"Others")) %>% 
  ggplot(aes(Corr,fill = Colour,colour = Colour))+
      scale_color_identity(labels = c(blue = .x,gray = "Others"),guide = "legend")+
    scale_fill_identity(labels = c(blue = .x,gray = "Others"))+
    geom_density(alpha = 0.1) +
...
PivotDasetTissue <- function(Dataset){ # this is for proteomics, adapt to RNAseq
  ...
  list1 <- map(lineage_types, 
               ~select(as.data.frame(Dataset),any_of(sample_info_dataset$stripped_cell_line_name[sample_info_dataset$lineage == .x]))) %>% 
    set_names(lineage_types)
...
```{r marker_gene_corr}
TranslationalEfficiency <- function(dataset1,dataset2){
  #create tests lineage type and cell lines?
  ...
  Tissue_Translation_Ratio_test <- Tissue_Translation_Ratio %>% group_split(Uniprot)
  .x <- Tissue_Translation_Ratio_test[[1]]
  eIF_dataset1 <- dataset1 %>% .[rownames(.)== "Q9NWT6",]
  
  eIF_dataset1 %>% select(one_of(.x$Cell_line)) %>% .[,match(.x$Cell_line,colnames(.))]
