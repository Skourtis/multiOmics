...
IntraFeature_NCI <- IntraFeatureCorrelation(NCI_60_proteins,
                                             NCI_60_RNA)  
IntraFeature_CCLE[str_detect(names(IntraFeature_CCLE),"plot|grid")] %>% map(print)
IntraFeature_NCI[str_detect(names(IntraFeature_NCI),"plot|grid")] %>% map(print)
```
...
#     pivot_longer(-Uniprot, names_to = "Cell_line", values_to = "Abundance"), .id = .x)
# }
...
#   map(markers, ~Dataset %>% filter(Uniprot == .x) %>% group_by(bone) %>% 
#         ggplot(aes(x = bone, colour = bone,
#                    y = Abundance)) + 
#     geom_boxplot() + 
#       geom_point() +
...
#   pull(Uniprot) %>% unique()
# Uniprot_eIF <- HUMAN_9606_idmapping %>% filter(str_detect(ID, 'EIF') & Type == "UniProtKB-ID") %>% 
#   pull(Uniprot) %>% unique()
# 
# 
...
Per_eIF_corr <-  map(unique(eIF_correlations$Factor), 
                                 ~ggplot(eIF_correlations[eIF_correlations$Factor ==.x,],
                                   ...
                                   geom_density(alpha = 0.05) +
                                   ggtitle(paste("eIF Corr by Pathway",.x))) %>% 
    set_names(unique(eIF_correlations$Factor))

map(Per_eIF_corr, plot)
