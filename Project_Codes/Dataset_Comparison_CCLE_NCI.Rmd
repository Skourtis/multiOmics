...
pacman::p_load(knitr,cgwtools,tidyverse, MatrixCorrelation)
...
IntraFeatureCorrelation <- function(dataset1,dataset2){
  ...
  Correlation_intrafeature <- function(x){
    Correlation_pairwise(dataset1[rownames(dataset1) == x,] %>% unlist(),
                         dataset2[rownames(dataset2) == annotation$ID[annotation$Uniprot == x],] %>% unlist())
  }
  
  listing <- map(Matching, safely(Correlation_intrafeature)) %>% 
    set_names(Matching) %>% map("result") %>%
    compact() %>% unlist() %>% stack() %>% 
    setNames(c("Corr","Uniprot")) %>% 
    left_join(Master_pathways, by = c("Uniprot" = "ID"))
  
  p <- ggplot(listing,aes(Corr,fill = Pathway, colour = Pathway)) +
         geom_density(alpha = 0.05) +
        ggtitle(paste("IntraFeature Corr by Pathway",Comparison_type))
  
  list(IntraFeature_data = listing, IntraFeature_plot = p)
  }
